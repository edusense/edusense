...
if __name__ == '__main__':
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        process = subprocess.Popen([
            ...
            stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()
        front_url_container_id = stdout.decode('utf-8').strip()

        print('created front video container', front_url_container_id)
        ...
        process = subprocess.Popen([
            ...
            stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()
        back_url_container_id = stdout.decode('utf-8').strip()

        print('created back video container', back_url_container_id)
        ...
        process = subprocess.Popen([
            ...
            '-e', 'LOCAL_USER_ID=%s' % uid,
            '-v', '%s:/tmp' % tmp_dir,
            'edusense/openpose:deploy',
            '--ip_camera', args.front_url,
        ...
        timer = Timer(7200, kill_all_containers, ([
            front_url_container_id,
            front_openpose_container_id,
            back_url_container_id,
