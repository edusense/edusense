...
def mount_get_classes(logger):
    ...
    for subdir, dirs, files in allfiles:
        for filename in files:
            ...
            if 'avi' in filepath and ((args.keyword=='all') | (args.keyword in filepath)):
...
if __name__ == '__main__':
    # try:
    #     os.remove('schedule collector.log')
    # except:
    #     pass
    #
    # try:
    ...
    # logging.basicConfig(filename='schedule collector.log', level=logging.DEBUG)
    ...
    parser.add_argument('--log_dir', dest='log_dir', type=str, nargs='?',
                        required=False, help='Log directory to collect schedule collector logs', default=0)
    ...
    core_logging_handler = WatchedFileHandler(f"{args.log_dir}/schedule_collector_{logging_dict['keyword']}.log")
    ...
    logger.info("--------------------------Starting new schedule collector session at %s--------------------------", time_str)
    ...
    logger.info("Created directory %s for this schedule collector session", directory)
    ##[[], [], [], []]
